...
export const NetCalculationCard: React.FC<NetCalculationCardProps> = ({
  ...
  return (
    <Card className="bg-card border border-orange-200/50 dark:border-orange-800/30 shadow-md hover:shadow-lg overflow-hidden relative group transition-shadow duration-300">
      ...
      <CardContent className="p-4 sm:p-6 pt-0 space-y-3 relative z-10">
        {netResult.type === 'insufficient_users' && (
          <div className="text-center p-4 bg-muted/60 rounded-xl border border-border/50">
            <div className="text-sm text-muted-foreground">{netResult.message}</div>
          </div>
        )}

        {netResult.type === 'balanced' && (
          <div className="p-4 bg-green-100/80 dark:bg-green-900/30 rounded-xl border border-green-200/50 dark:border-green-700/50">
            <div className="flex items-center justify-center gap-3 mb-2">
              <CheckCircle className="h-6 w-6 text-green-600 dark:text-green-400" />
              <div className="font-bold text-green-700 dark:text-green-300 text-lg">{netResult.message}</div>
            </div>
            <div className="text-xs text-green-600/80 dark:text-green-400/80 p-2 rounded-lg text-center">
              אין צורך בהעברת כסף
            </div>
          </div>
        )}
        ...
        {netResult.type === 'transfer' && (
          <div className="p-4 bg-orange-100/80 dark:bg-orange-900/30 rounded-xl border border-orange-200/50 dark:border-orange-700/50">
            ...
            <div className="flex items-center gap-2 justify-center">
              ...
              <div className="text-xs text-orange-600/80 dark:text-orange-400/80 px-3 py-1 rounded-full font-mono">
                ₪{netResult.amount}
              </div>
            </div>
          </div>
