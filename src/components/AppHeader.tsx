...
const AppHeader: React.FC<AppHeaderProps> = ({ onMenuClick, isMobile }) => {
  ...
  return (
    <header className="liquid-glass border-b border-white/40 dark:border-white/10 px-3 sm:px-4 py-3 sticky top-0 z-30">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          {isAuthenticated && (
            <>
              {/* AccountSwitcher + ThemeToggle hide on mobile to keep the top
                  bar uncluttered — they're surfaced inside the hamburger
                  drawer (AppSidebar) instead. Notification + Logout stay
                  visible because they're high-frequency. */}
              {!isMobile && (
